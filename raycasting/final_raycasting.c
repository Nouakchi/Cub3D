...
void draw_line(t_data *data, int x_start, int y_start, int y_end, int actual_height)
{
	...
	if (actual_height > W_HEIGHT)
		total = (((float) actual_height - W_HEIGHT) / 2.0) * (WALL_HEIGHT / (float) actual_height);
	in = (int)total;
	if (y_end >= W_HEIGHT)
		y_end = W_HEIGHT - 1;
	if (y_start < 0)
		y_start = 0;
	color = (data->map_data.map_elements.crgb.r * pow(2,16)) + (data->map_data.map_elements.crgb.g * pow(2,8)) + data->map_data.map_elements.crgb.b;
	i = 0;
	while (i < y_start)
	...
	color = (data->map_data.map_elements.frgb.r * pow(2,16)) + (data->map_data.map_elements.frgb.g * pow(2,8)) + data->map_data.map_elements.frgb.b;
...
void	get_first_vert_inter(t_data *data, int *i, int *j)
{
    ...
    data->ray.y_step = fabs(WALL_HEIGHT * tan(data->ray.angle * (M_PI / 180.0)));
	tmp = 0;
	if ((data->ray.angle < 90 || data->ray.angle >= 270))
		tmp = WALL_HEIGHT;
